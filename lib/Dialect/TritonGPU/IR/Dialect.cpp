...
Attribute PPUMmaEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  std::optional<CGAEncodingAttr> CGALayout =
      parseCGAAttr(parser, ctaAttr, /*rank=*/warpsPerCTA.size());
  if (!CGALayout.has_value())
    return {};
...
Attribute PPUAIUSharedEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  std::optional<CGAEncodingAttr> CGALayout =
      parseCGAAttr(parser, ctaAttr, /*rank=*/order.size());
  if (!CGALayout.has_value())
    return {};

  return parser.getChecked<PPUAIUSharedEncodingAttr>(
      parser.getContext(), versionMajor, AIUStrategy, order, *CGALayout,
      kOffset);
}
...
LogicalResult DotOperandEncodingAttr::verify(
    ...
    Attribute parent, unsigned kWidth, bool isChained) {
  if (opIdx != 0 && opIdx != 1) {
