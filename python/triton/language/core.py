...
class _block_ptr:
    ...
    def __init__(self, base, shape, strides, offsets, block_shape, order, _base_ptr_changed=False, _is_aiu=None,
                 _semantic=None):
        ...
        original_shape = [_unwrap_if_constexpr(value) for value in _as_list_like(shape)]
        original_strides = [_unwrap_if_constexpr(value) for value in _as_list_like(strides)]
        self.base = base
        ...
        if _is_aiu is None:
            promote_use_aiu = _semantic.builder.options.backend_name == "ppu" and rank == 2
            if promote_use_aiu:
                contiguous_dim = order_values[0]
                non_contiguous_dim = order_values[1]
                elem_size = base.dtype.element_ty.primitive_bitwidth // 8
                contiguous_stride = original_strides[contiguous_dim]
                contiguous_extent = _unwrap_if_constexpr(self.block_shape[contiguous_dim])
                non_contiguous_extent = _unwrap_if_constexpr(self.block_shape[non_contiguous_dim])
                contiguous_shape = original_shape[contiguous_dim]
                non_contiguous_stride = original_strides[non_contiguous_dim]
                promote_use_aiu = (
                    isinstance(contiguous_stride, int) and contiguous_stride == 1 and elem_size == 2
                    and contiguous_extent * elem_size >= 32 and non_contiguous_extent % 16 == 0
                    and isinstance(contiguous_shape, int) and isinstance(non_contiguous_stride, int)
                    and contiguous_shape == non_contiguous_stride and not _unwrap_if_constexpr(_base_ptr_changed)
                )
            if os.getenv("PPU_DISABLE_AIU_PROMOTION", "").upper() in {"ON", "1", "YES", "TRUE", "Y"}:
                promote_use_aiu = False
        else:
            promote_use_aiu = _unwrap_if_constexpr(_is_aiu)
        self._is_aiu = constexpr(promote_use_aiu)
    ...
    def advance(self, offsets, _semantic=None):
        ...
        return _block_ptr(self.base, self.shape, self.strides, tuple(new_offsets), self.block_shape, self.order,
                          _is_aiu=self._is_aiu, _semantic=_semantic)
    ...
    def load(self, mask=None, other=None, boundary_check=(), padding_option="", cache_modifier="", eviction_policy="",
        ...
        if self._is_aiu.value and not checked_dims and padding_option == "" and not volatile:
            result_type = block_type(self.base.type.element_ty, self._tile_shape())
            return _semantic.aiu_load(self.base, self.offsets, self.shape, self.order, cache_modifier,
                                      eviction_policy, result_type)
...
def aiu_load(pointer, offsets=(0, 0), block_shape=(0, 0), shape=(0, 0), dtype=void, order=(1, 0), _semantic=None):
    if _is_block_ptr(pointer):
        ...
        block_shape = pointer.block_shape
        shape = pointer.shape
        dtype = pointer.base.type.element_ty
        order = pointer.order
        pointer = pointer.base

    dtype = _unwrap_if_constexpr(dtype)
    block_shape = _unwrap_shape(block_shape)
    result_type = block_type(dtype, block_shape)
    return _semantic.aiu_load(pointer, offsets, shape, order, "", "", result_type)
