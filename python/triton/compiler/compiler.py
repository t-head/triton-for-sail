...
def make_backend(target: GPUTarget) -> BaseBackend:
    ...
    if selected:
        if selected not in backends:
            raise RuntimeError(f"Unknown backend '{selected}'. Available backends: {list(backends.keys())}")
        backend = backends[selected].compiler
        if not backend.supports_target(target):
            raise RuntimeError(f"Backend '{selected}' does not support target '{target.backend}'.")
