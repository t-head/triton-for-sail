...
@triton.jit
def matmul_kernel_aiu_fp32(A_ptr, B_ptr, C_ptr,
                                M, N, K,
                                stride_am, stride_ak,
                                stride_bk, stride_bn,
                                stride_cm, stride_cn,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K)):
        a = tl.aiu_load(A_ptr, (offs_am, offs_k), (BLOCK_M, BLOCK_K), (M, K), tl.float32)
        b = tl.aiu_load(B_ptr, (offs_k, offs_bn), (BLOCK_K, BLOCK_N), (K, N), tl.float32)
...
def test_aiu_matmul_fp32(num_stages, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps):
    device = "cuda"
    torch.manual_seed(0)
    ...
    C_tri_aiu = torch.empty((M, N), device=A.device, dtype=torch.float32)
    C_tri = torch.empty((M, N), device=A.device, dtype=torch.float32)
    C_ref = torch.matmul(A, B)

    grid = (triton.cdiv(M, BLOCK_M) * triton.cdiv(N, BLOCK_N), 1, 1)
    matmul_kernel_aiu_fp32[grid](
        A, B, C_tri_aiu,
        M, N, K,
        A.stride(0), A.stride(1),
        B.stride(0), B.stride(1),
        C_tri_aiu.stride(0), C_tri_aiu.stride(1),
        BLOCK_M=BLOCK_M,
        BLOCK_N=BLOCK_N,
        BLOCK_K=BLOCK_K,
        num_warps=num_warps,
    ...
    max_diff = (C_tri_aiu - C_tri).abs().max().item()
    torch.set_printoptions(sci_mode=False, precision=6)

    print(f"Max diff: {max_diff:.6e}")
    print(f"allclose: {torch.allclose(C_tri_aiu, C_tri, atol=1e-3, rtol=1e-3)}")
    # print(C_tri)
    # print(C_ref)
    np.testing.assert_allclose(to_numpy(C_tri), to_numpy(C_ref), atol=1e-1, rtol=1e-1)
    np.testing.assert_allclose(to_numpy(C_tri_aiu), to_numpy(C_ref), atol=1e-1, rtol=1e-1)
    np.testing.assert_allclose(to_numpy(C_tri), to_numpy(C_tri_aiu), atol=1e-3, rtol=1e-3)

# test_aiu_matmul_fp32(1, 512, 512, 512, 32, 32, 32, 1)